<%@page import="obj.StudentStatusModify"%>
	...
	<div id="wrapper">
		...
		<div id="main">
			<div class="inner">
				...
				<!-- Header -->
				<header id="header">
					<a href="index.html" class="logo"><strong>학적변동내역 조회</strong></a>
					<ul class="icons">
				...
				<section id="banner">
					<div class="content">
						...
						<table>
						<thead>
						<tr><th>No</th><th>년도/학기</th><th>신청일자</th><th>신청구분</th></tr>
						</thead>
						<tbody>
						<%
							int no= 1;	// 번호
							StudentStatusModifyHistoryDAO studentStatusModifyHistoryDAO = new StudentStatusModifyHistoryDAO();
							List<StudentStatusModify> statusModifyList = studentStatusModifyHistoryDAO.getStatusModifyList(identifier);
							
							
							for (StudentStatusModify statusModify : statusModifyList){
							%>
							<tr><td><%=no %></td><td><%=statusModify.getYear_and_semester() %></td><td><%=statusModify.getModified_date() %></td><td><%=statusModify.getModified_status() %></td></tr>
							<%
							no++;
							}
						%>
